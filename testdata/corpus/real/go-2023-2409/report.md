# Unbounded PBES2 iteration count in jose2go lets a token burn CPU for minutes

jose2go (v1.5.0, commit c5d6a57d4843d9cc91403187d16eea0a70f64db4) trusts the `p2c` (PBES2 iteration count) header of
an incoming JWE token.

`(*Pbse2HmacAesKW).Unwrap` in `pbse2_hmac_aeskw.go` reads `p2c` from the protected
header and passes it straight to PBKDF2 before the key is checked. `Decode` in
`jose.go` reaches this for any token whose `alg` is one of the `PBES2-*` algorithms.
An attacker who can submit a token (no key needed) sets `p2c` to 2^31-1 and the
server spends minutes of CPU per request deriving a key that is then thrown away.

Impact: remote denial of service against any service that decodes JWE tokens with
jose2go and a password-based key.

## Reproduce

Drop this test into the repository root at commit `c5d6a57d4843d9cc91403187d16eea0a70f64db4` and run `go test -run TestPoC -count=1 .` from that directory. It fails (or panics) on the affected code and passes once the issue is fixed.
